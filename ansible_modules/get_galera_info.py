...
def get_safe_to_bootstrap(database_port):
...
def get_seqno(database_port):
    seqno = 0
    for line in os.popen(". /etc/profile; mysqld_safe --defaults-file=/mysqldata/%s/my.cnf --wsrep_recover" % database_port).readlines():
        if "Recovered position" in line:
            seqno = line.split(":")[-1].strip().replace("\n","")
    return  seqno

# 检查数据库是否存活,1存活，0异常
def get_is_alive(database_port):
    is_alive = 0
    try:
        cnx = MySQLdb.connect(host='127.0.0.1', user='dic_wh', passwd='tydic123', port=int(database_port))
        is_alive = 1
        cnx.close()
    except Exception, e:
        pass
...
def main():
    ...
    database_info['is_alive'] = get_is_alive(module.params['port'])
    #若数据库不存活，则检测safe_to_boostrap与last commited no
    if database_info['is_alive'] == 0:

        database_info['safe_to_bootstrap'] = get_safe_to_bootstrap(module.params['port'])

        #若节点不允许安全启动（safe_to_bootstrap=0），那么检测last commited no
        if database_info['safe_to_bootstrap'] == '0':
            database_info['seqno'] = get_seqno(module.params['port'])
        else:
